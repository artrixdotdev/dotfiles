...
Singleton {
   ...
   function updateCurrentOutputWorkspaces() {
      currentOutputWorkspaces = currentOutput ? allWorkspaces.filter(w => w.output === currentOutput) : allWorkspaces;
   }
   ...
   function getCurrentWorkspaceNumber() {
      return (focusedWorkspaceIndex >= 0 && focusedWorkspaceIndex < allWorkspaces.length) ? allWorkspaces[focusedWorkspaceIndex].idx + 1 : 1;
   }
